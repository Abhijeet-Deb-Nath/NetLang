 ...
 * 
 * Reads preprocessed .bin files and runs inference using generated network code.
 * This provides a clean interface for testing networks with real data.
 * 
 * Usage:
 *   test_network input.bin
 *   test_network test_data/preprocessed/digit_7.bin
 * 
 * Compile:
...
float* load_bin_file(const char* filename, size_t expected_size) {
    ...
    long expected_bytes = expected_size * sizeof(float);
...
void print_probabilities(const float* probs, int size) {
    ...
    for (int i = 0; i < size; i++) {
        int bar_width = (int)(probs[i] * 50);  // Scale to 50 chars
...
int main(int argc, char** argv) {
    if (argc < 2) {
        ...
        printf("\nUsage: %s <input.bin>\n", argv[0]);
        printf("\nExample:\n");
        printf("  %s test_data/preprocessed/digit_7.bin\n", argv[0]);
